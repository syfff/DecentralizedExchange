...
const exchange_name = 'MMSWAP';             // TODO: fill in the name of your exchange

const token_name = 'TOKEN_NAME';                // TODO: replace with name of your token
const token_symbol = 'TN';              // TODO: replace with symbol for your token
...
async function addLiquidity(amountEth, maxSlippagePct) {
  ...
  exchange_price = poolState["eth_token_rate"]
  let max_exchange_rate = Math.round(exchange_price * (100 + maxSlippagePct));
  let min_exchange_rate = Math.round(exchange_price * (100 - maxSlippagePct));
  let amountToken = Math.round(poolState['token_eth_rate'] * amountEth);
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  const poolState = await getPoolState();
  exchange_price = poolState["token_eth_rate"];
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  exchange_price = poolState["eth_token_rate"];
  console.log(exchange_price);
  let max_exchange_rate = Math.round(100 * (exchange_price + maxSlippagePct));
...
const sanityCheck = async function () {
  ...
  // LP provider rewards implemented
    ...
    console.log('entering 4th test');


    var state4 = await getPoolState();

    console.log(state4.eth_liquidity);
    var user_tokens4 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
    await removeLiquidity(10, 1);
    console.log('after removing 4th test');
    ...
    // var aa = await exchange_contract.get_lpa__(defaultAccount);
    // console.log(aa);
    // console.log(defaultAccount);
    //console.log(state5.eth_liquidity);
    await removeAllLiquidity(1);
